  ...
  allGraphs(allGraphs)
{
  ...
  commands.insert(Pair("ADD", &mitkov::Command::add));
  commands.insert(Pair("RADD", &mitkov::Command::rAdd));
...
void mitkov::Command::topologicSort(const std::list<std::string>& args)
{
  ...
  std::list< std::string >::const_iterator it = args.begin();
  orientedGraph< int >& affected = allGraphs.find(*it++)->second;
  std::vector< int > result = affected.topologicSort();
  for (auto& x: result) {
    if (x != *result.begin()) {
      stream << ' ';
    }
...
void mitkov::Command::getCycle(const std::list<std::string>& args)
{
  ...
  orientedGraph< int >& affected = allGraphs.find(*it++)->second;
  std::vector< int > result = affected.topologicSort();
  for (graph< int, true >::iterator it = affected.begin(); it != affected.end(); ++it) {
    if (it != affected.begin()) {
      stream << ' ';
    }
    stream << *it;
  }
...
void mitkov::Command::add(const std::list<std::string>& args)
{
  if (args.size() != 3) {
    throw std::invalid_argument("Not enough data");
  }
  std::list< std::string >::const_iterator it = args.begin();
  orientedGraph< int >& affected = allGraphs.find(*it++)->second;
  int attachedTo = std::stoi(*it++);
  int key = std::stoi(*it);
  affected.add(attachedTo, key);
}

void mitkov::Command::rAdd(const std::list<std::string>& args)
{
  if (args.size() != 3) {
    throw std::invalid_argument("Not enough data");
  }
