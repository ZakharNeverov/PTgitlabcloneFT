...
6)compressionAllFiles - вводится два файла, из сообщений внутри файлов строится
новое сообщение и записывается в файл с названием mergeMessageCode, который кодируется.
7)delta - вводится имя файла с сообщением. Выводится начальный размер файла
и размер его сжатой версии.
8)isFileEncrypted - вводится имя файла с сообщением.
Выводится "True", если закодированный файл с таким именем и расширением .hff существует.
Иначе "False", если такого файла не существует.
Пример команд:
1)readFile file1
2)fillFreq file2
