...
3)printCodeDiff - вводится имя файла с сообщением, затем выводится исходный текст, после - зашифрованный в квадратных скобках
...
5)compressionFiles - вводится два файла и кодируются отдельно. Зашифрованные файлы будут иметь имя оригинального файла,
  но с расширением .hff
6)compressionAllFiles - вводится два файла, из сообщений внутри файлов строится новое сообщение и записывается в файл
  mergeMessageCode, который кодируется.
7)delta - вводится имя файла с сообщением. Выводится начальный размер файла и размер его сжатой версии.
8)isFileEncrypted - вводится имя файла с сообщением. Выводится "True", если закодированный файл с таким именем и расширением .hff 
  существует. "False", если такого файла не существует.
